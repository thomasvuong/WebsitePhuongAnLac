...
export default function Features() {
  ...
  const [currentText3, setCurrentText3] = useState(0)
  ...
  const alternatingTexts3 = [
    {
      title: "Ủy ban nhân dân",
      subtitle: "Cơ quan hành chính nhà nước",
      content: ""
    },
    {
      title: "UBND phường An Lạc đẩy mạnh chính quyền số", 
      subtitle: "Hoàn thành ≥90% dịch vụ công trực tuyến hướng tới mô hình \"phường thông minh\".",
      content: ""
    }
  ]
  ...
  useEffect(() => {
    ...
    const interval3 = setInterval(() => {
      setCurrentText3((prev) => (prev + 1) % alternatingTexts3.length)
    }, 3000) // Change text every 3 seconds
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow p-6">
            ...
            <div className="transition-all duration-500 ease-in-out">
              <h3 className="text-xl font-bold mb-2">{alternatingTexts3[currentText3].title}</h3>
              <p className="text-gray-600 mb-2 font-semibold">{alternatingTexts3[currentText3].subtitle}</p>
              {alternatingTexts3[currentText3].content && (
                <p className="text-gray-600 mb-4">{alternatingTexts3[currentText3].content}</p>
              )}
            </div>
            <a href="#" className="text-red-600 hover:text-red-700 font-medium flex items-center gap-2">
              Xem chi tiết <ArrowRight className="w-4 h-4" />
